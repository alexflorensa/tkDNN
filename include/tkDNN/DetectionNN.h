...
namespace tk {
    namespace dnn {
        ...
        public:
             ...
             */
            virtual bool init(const std::string &tensor_path, const int n_classes = 80, const int n_batches = 1,
                              const float conf_thresh = 0.3) = 0;
            ...
            void update(std::vector<cv::Mat> &frames, const int cur_batches = 1, bool save_times = false,
                ...
                if (TKDNN_VERBOSE) printCenteredTitle(" TENSORRT detection ", '=', 30);
                {
                    ...
                    for (int bi = 0; bi < cur_batches; ++bi) {
                        if (!frames[bi].data) FatalError("No image data feed to detection");
            ...
            void draw(std::vector<cv::Mat> &frames) {
                ...
                for (int bi = 0; bi < frames.size(); ++bi) {
                    ...
                    for (int i = 0; i < batchDetected[bi].size(); i++) {
                        ...
                        // draw label
                        cv::Size text_size = getTextSize(det_class, cv::FONT_HERSHEY_SIMPLEX, font_scale, thickness,
                                                         &baseline);
                        cv::rectangle(frames[bi], cv::Point(x0, y0),
                                      cv::Point((x0 + text_size.width - 2), (y0 - text_size.height - 2)), colors[b.cl],
                                      -1);
                        cv::putText(frames[bi], det_class, cv::Point(x0, (y0 - (baseline / 2))),
